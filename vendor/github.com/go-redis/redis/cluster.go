...
type ClusterOptions struct {
	...
	// Enable changing remapped node IP addresses back to the
	// configured ones.
	// Be careful not to set this value if your node IPs are not remapped,
	// this forces all internal IPs that are not the same as configured to be
	// the configured ones even if the cluster nodes have changed.
	FixIPRemapping bool
...
func (opt *ClusterOptions) init() {
	...
	// init private IP ranges for test
...
func newClusterState(
	...
	for _, slot := range slots {
		...
		for i, slotNode := range slot.Nodes {
			...
			// fix IP remapping
			if c.nodes.opt.FixIPRemapping && addrHost != originHost {
				replacedAddr, err := fixIPRemapping(addr, originHost)
				if err != nil {
					return nil, err
				}
				addr = replacedAddr
...
func isValidIP(s string) bool {
	...
	reIPV4 := regexp.MustCompile(`^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?` +
		`)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$`)
	reIPV6 := regexp.MustCompile(`^(([0-9a-fA-F]{1,4}:){7,7}[0-9a-fA-F]{1,4}|` +
		...
		`{1,7}|:)|fe80:(:[0-9a-fA-F]{0,4}){0,4}%[0-9a-zA-Z]{1,}|` +
	...
	if reIPV4.Match([]byte(s)) || reIPV6.Match([]byte(s)) {
		return true
	}
	return false
}

func fixIPRemapping(nodeAddr, originHost string) (string, error) {
	// If configured originHost is not IP addr, nodeAddr will be left as it is
	if !isValidIP(originHost) {
		return nodeAddr, nil
	}

	// Don't fix when configured node IPs are also private.
	// Even if the node IPs have changed, by a good chance we can still access
	// it when the configured ones are also private.
	originIP := net.ParseIP(originHost)
	if originIP == nil {
		return "", errors.New("Invalid IP addr " + originHost +
			" for node")
	}
	if isPrivateIP(originIP) {
		return nodeAddr, nil
	}
	...
	if ip == nil {
		return "", errors.New("Invalid IP addr " + nodeAddr +
			" for node")
	}
	if !isPrivateIP(ip) {
		return nodeAddr, nil
	}
	res := net.JoinHostPort(originHost, nodePort)
	// fmt.Println("replaced " + nodeAddr + " to " + res)
	return res, nil
}
